...
public class ComponentLoader<T> {
	...
	public T getComponent(final String url) {
		...
		Iterator<String> patterns = this.urlToName.keySet().iterator();
		String name = null;
	...
	private void loadComponentDefines(final Document config)
		...
		if (size == 1) {
			...
			nodelist = componentDefines.getElementsByTagNameNS(
					ComponentLoader.NS, "component");
			size = nodelist.getLength();
			if (size > 0) {
				for (int i = 0; i < size; i++) {
					Element componentElement = (Element) nodelist.item(i);
					NodeList nodes = componentElement.getElementsByTagNameNS(
							ComponentLoader.NS, "component-name");
					if (nodes.getLength() == 0) {
						continue;
					}
					String name = nodes.item(0).getTextContent();
					nodes = componentElement.getElementsByTagNameNS(
							ComponentLoader.NS, "component-class");
					if (nodes.getLength() == 0) {
						continue;
					}
					String className = nodes.item(0).getTextContent();
					nodes = componentElement.getElementsByTagNameNS(
							ComponentLoader.NS, "load-on-startup");
	...
	private void loadComponentMappings(final Document config)
		...
		if (size == 1) {
			...
			nodeList = componentMappings.getElementsByTagNameNS(
					ComponentLoader.NS, "component-mapping");
			size = nodeList.getLength();
		}
		if (size > 0) {
			for (int i = 0; i < size; i++) {
				Element componentMapping = (Element) nodeList.item(i);
				NodeList nodes = componentMapping.getElementsByTagNameNS(
						ComponentLoader.NS, "component-name");
				if (nodes.getLength() == 0) {
					continue;
				}
				String name = nodes.item(0).getTextContent();
				nodes = componentMapping.getElementsByTagNameNS(
						ComponentLoader.NS, "url-pattern");
